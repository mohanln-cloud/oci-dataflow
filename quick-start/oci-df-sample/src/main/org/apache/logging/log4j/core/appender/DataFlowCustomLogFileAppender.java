 ...
 */
@Plugin(
        name = "DataFlowCustomLogFileAppender", //RollingFileAppender.PLUGIN_NAME,
        category = Core.CATEGORY_NAME,
        elementType = Appender.ELEMENT_TYPE,
        printObject = true)
public final class DataFlowCustomLogFileAppender extends AbstractOutputStreamAppender<RollingFileManager> {

    public static final String PLUGIN_NAME = "DataFlowCustomLogFileAppender";

    private static final String RUN_ID = null;
    private static final String RUN_NAME = null;
    private static final String OPC_REQUEST_ID = null;

    static final TimeZone UTC = TimeZone.getTimeZone("UTC");
    static final FastDateFormat ISO_DATETIME_TIME_ZONE_FORMAT_WITH_MILLIS = FastDateFormat
    ...
    public static class Builder<B extends Builder<B>> extends AbstractOutputStreamAppender.Builder<B>
            implements org.apache.logging.log4j.core.util.Builder<DataFlowCustomLogFileAppender> {
        ...
        @Override
        public DataFlowCustomLogFileAppender build() {
            if (!isValid()) {
                return null;
            }
            ...
            if (!isBufferedIo && bufferSize > 0) {
                LOGGER.warn(
                        "DataFlowRollingFileAppender '{}': The bufferSize is set to {} but bufferedIO is not true",
                        getName(),
                        bufferSize);
            }
            ...
            if (strategy == null) {
                ...
                LOGGER.error(
                        "DataFlowRollingFileAppender '{}': When no file name is provided a {} must be configured",
                        getName(),
            ...
            return new DataFlowCustomLogFileAppender(
                    getName(),
                    layout,
                    getFilter(),
                    manager,
    ...
    private DataFlowCustomLogFileAppender(
            final String name,
    ...
    public void append(final LogEvent event) {
        ...
        super.append(event);
        System.out.print("\nInside DataFlowCustomLogFileAppender log" + event.getMessage().getFormattedMessage());
    /*    ServiceLogEntry entry = ServiceLogEntry.builder()
                .id(UUID.randomUUID().toString())
                .time(dateFormat(event.getTimeMillis()))
                .content(ServiceLogEntry.ServiceLogEntryContent.builder()
                        .runId(RUN_ID)
                        .opcRequestId(OPC_REQUEST_ID)
                        .message(event.getMessage().getFormattedMessage())
                        .logLevel(event.getLevel().toString())
                        .thread(event.getLoggerName())
                        .build())
                .build();
        if (logBuffer.append(entry)) {
    ...
    /**
     * Creates a DataFlowRollingFileAppender.
     ...
     * @return A DataFlowRollingFileAppender.
     * @deprecated Use {@link #newBuilder()}.
     */
    @Deprecated
    public static <B extends Builder<B>> DataFlowCustomLogFileAppender createAppender(
            // @formatter:off
            final String fileName,
            final String filePattern,
            final String append,
        ...
        // @formatter:off
        return DataFlowCustomLogFileAppender.<B>newBuilder()
