...
public class StructuredKafkaWordCount {
    ...
    public static void main(String[] args) throws Exception {
      ...
      /*  StructType wordsSchema = StructType$.MODULE$.apply(
        ...
        ExpressionEncoder<Row> encoder = (ExpressionEncoder<Row>) Encoders.kryo(Row.class);
        ...
        */
        ...
        switch (type) {
            case "console":
                query = outputToConsole(lines, checkpointLocation);
                break;
            case "csv":
                query = outputToCsv(lines, checkpointLocation, outputLocation);
                break;
            case "delta":
                query = outputToDelta(lines, checkpointLocation, outputLocation);
                break;
            default:
    ...
    private static StreamingQuery outputToDelta(Dataset<Row> wordCounts, String checkpointLocation,
                                              String outputLocation) throws TimeoutException {
        return wordCounts
                .writeStream()
                .format("csv")
