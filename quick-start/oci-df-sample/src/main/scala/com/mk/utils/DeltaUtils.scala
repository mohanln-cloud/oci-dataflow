...
object DeltaUtils {
  ...
  def getSparkSession(): SparkSession = {
    val spark = SparkSession
      .builder()
      .appName("DataFlow Simulation App")
...
//     .master("local[1]")
  ...
  def StartDeltaStreamSink(format:String, inputPath: String, outputPath: String, checkpoint: String) = {

    val spark = getSparkSession()

      spark
      .readStream.format(format)//.option("header", true).option("inferSchema", true)
      .load(inputPath)
  ...
  def generateData(inputFormat:String, inputPath: String, outputForamt: String, outputPath: String) = {
    ...
    val original_df = spark
      ...
      original_df.write.mode("append").format(outputForamt).save(outputPath)
